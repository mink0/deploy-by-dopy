...
exports.command = 'sync [targets]';

exports.desc = 'Synchronize remote application with remote repository';
...
exports.task = (env, argv, taskCb) => {
  ...
  function targetsLoop(targetsCb) {
    ...
    // else
    //   tasks.push(cb => targetProcessor(env, cb));
    ...
    function makeReport(cb) {
      ...
      target.remote('git describe --tags', { mute: true }, (err, res) => {
        ...
        target.remote('cat ./CHANGELOG.md', { mute: true }, (err, res) => {
          ...
          let out = [];
          out.push(`<b>Релиз ${tag} для ${target.name} ` +
